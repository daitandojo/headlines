// File: app-logic.js (version 1.06)
...
function logFinalSummary(articles, articlesRelevanceThreshold, headlinesRelevanceThreshold) {
    ...
    relevantForSummary.forEach((article, index) => {
        pipelineLogger.info(`  Article #${index + 1}:`);
        pipelineLogger.info(`    - Headline: ${truncateString(article.headline, 100)}`);
        pipelineLogger.info(`    - Link: ${article.link}`);
        pipelineLogger.info(`    - Source: ${article.newspaper || article.source || 'N/A'}`);
        pipelineLogger.info(`    - Headline Relevance: ${article.relevance_headline ?? 'N/A'} (Assessment: ${truncateString(String(article.assessment_headline ?? ''), 80) || 'N/A'})`);
        if (article.relevance_article !== undefined && article.relevance_article !== null) {
            pipelineLogger.info(`    - Article Relevance: ${article.relevance_article} (Assessment: ${truncateString(String(article.assessment_article ?? ''), 80) || 'N/A'})`);
        } else if (article.error || article.enrichment_error) {
            pipelineLogger.info(`    - Article Relevance: Not assessed due to content issues.`);
        }
        if (article.error) pipelineLogger.warn(`    - Processing Error: ${article.error}`);
        if (article.enrichment_error) pipelineLogger.warn(`    - Enrichment Error: ${article.enrichment_error}`);
        if (article.emailed) pipelineLogger.info(`    - Email Status: Sent`);
        else if (article.email_error) pipelineLogger.warn(`    - Email Status: Error - ${article.email_error}`);
        else if (article.email_skipped_reason) pipelineLogger.info(`    - Email Status: Skipped - ${article.email_skipped_reason}`);
        pipelineLogger.info('----------------------------------------------------------------------');
    });
}

export async function executePipeline() {
  // --- FIX: Added a console.log at the very start for reliable execution confirmation ---
  console.log('[PIPELINE] executePipeline function has been entered.');
    ...
    const workflowSteps = [
        {
            ...
            postProcess: (result) => {
                ...
                if (!result || result.length === 0) { logPipelineDuration(startTime, '⏹️ No headlines fetched.'); return null; }
                pipelineLogger.info(`📰 Fetched ${result.length} headlines.`);
                return result;
            },
        },
        {
            ...
            postProcess: (result) => {
                ...
                if (!result || result.length === 0) { logPipelineDuration(startTime, '⏹️ No fresh articles to process.'); return null; }
                pipelineLogger.info(`🆕 Found ${result.length} fresh articles not in DB.`);
                return result;
            },
        },
        {
            ...
            postProcess: (result) => {
                ...
                const successfullyAssessed = result.filter((a) => a && !a.error);
                runStats.passedHeadlineThreshold = successfullyAssessed.filter( (a) => a.relevance_headline >= HEADLINES_RELEVANCE_THRESHOLD ).length;
                pipelineLogger.info(`🧐 ${successfullyAssessed.length} headlines AI-assessed. ${runStats.passedHeadlineThreshold} passed threshold.`);
                if (successfullyAssessed.length === 0 && result.length > 0) { logPipelineDuration(startTime, '⏹️ All articles had errors during headline AI assessment.'); return null; }
        ...
        },
        {
            ...
            postProcess: (result) => {
                if (!result || result.length === 0) { logPipelineDuration(startTime, '⏹️ No initial headline data to process for storage.'); return null; }
                const storedOrUpdated = result.filter((a) => a && !a.storage_error_initial_headline_data);
                runStats.dbInitialStoreSuccess = storedOrUpdated.length;
                const forEnrichment = storedOrUpdated.filter((a) => a.relevance_headline >= HEADLINES_RELEVANCE_THRESHOLD);
                if (forEnrichment.length === 0) { logPipelineDuration(startTime, `⏹️ No articles passed headline relevance (>=${HEADLINES_RELEVANCE_THRESHOLD}) for enrichment.`); return null; }
                pipelineLogger.info(`📝 ${forEnrichment.length} articles proceeding to content enrichment.`);
                return forEnrichment;
            },
        },
        ...
        { name: 'Assess Full Article Content', func: pipelineModules.assessArrayOfArticles, postProcess: (result) => {
            if (!result) return null;
            const forFinalStore = result.filter(article => {
                const hasError = !!(article.error && article.error !== 'Insufficient content') || !!article.enrichment_error;
                const isContentRelevant = !hasError && article.relevance_article >= ARTICLES_RELEVANCE_THRESHOLD;
                const isHeadlineSufficient = hasError && article.relevance_headline >= HEADLINES_RELEVANCE_THRESHOLD;
                return isContentRelevant || isHeadlineSufficient;
            });
            if (forFinalStore.length === 0) { pipelineLogger.info(`⏹️ No articles passed final relevance for storage.`); return result; }
            return forFinalStore;
        }},
